...
Public Property Let ButtonStyle(Style As BackStyleConstants)
    ...
    If myProps.bShape < lv_Round3D Or myProps.bShape < lv_CustomFlat And (myProps.bBackStyle = lv_hover Or lastStyle = lv_hover) Then CreateButtonRegion        ' re-create the button shape
...
Public Property Let Enabled(bEnabled As Boolean)
    ...
    If myProps.bBackStyle = 3 And myProps.bMode = lv_CommandButton And myProps.bShape < lv_Round3D Then
...
Public Property Let ShowFocusRect(bShow As Boolean)
    ...
    If ((myProps.bStatus And 1) = 1) Then
        ...
        Else
            ...
            MsgBox "The focus rectangle may appear on default buttons ONLY while in design mode, " & vbCrLf & "but will not appear when the form is running.", vbInformation + vbOKOnly
...
Public Property Let GradientColor(nColor As OLE_COLOR)
    ...
    If (myProps.bLockHover = lv_LockTextandBackColor Or myProps.bLockHover = lv_LockBackColorOnly) And myProps.bGradient > lv_NoGradient Then
...
Public Property Let HoverColorLocks(nLock As HoverLockConstants)
    ...
    If myProps.bLockHover = lv_LockTextandBackColor Or myProps.bLockHover = lv_LockBackColorOnly Then
...
Private Sub RedrawButton()
    ...
    Select Case myProps.bBackStyle
        ...
        Case 0
            DrawButton_Win95 polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 1
            DrawButton_Win31 polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 2
            DrawButton_WinXP polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 3
            DrawButton_Java polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 4
            DrawButton_Flat polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 5
            DrawButton_Hover polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 6
            DrawButton_Netscape polyPts(), polyColors(), ActiveStatus, ActiveClipRgn

        Case 7
            DrawButton_Macintosh polyPts(), polyColors(), ActiveStatus, ActiveClipRgn
...
Private Function ToggleOptionButtons(nMode As Integer) As Boolean
    ...
    Select Case nMode
        ...
        Case 2
            ' See if any option buttons have True values
...
Private Sub DrawButtonBackground(bColor As Long, ActiveStatus As Integer, ActiveRegion As Integer, Optional bGradientColor As Long = -1, Optional bHoverColor As Long = -1)
    ...
    If myProps.bGradient And myProps.bValue = False And myProps.bShape < lv_CustomFlat Then
        If bTimerActive = True And ((myProps.bStatus And 6) = 6) = False And (myProps.bGradientColor <> myProps.bBackHover) Then
...
Private Sub DrawCustomBorders(ActiveRegion As Integer)
    ...
    If bTimerActive = True Or ((myProps.bStatus And 6) = 6) Or myProps.bValue = True Or myProps.bBackStyle <> 5 Then
        ...
        If myProps.bShape < lv_RoundFlat Or myProps.bShape > lv_Round3D Then
            ...
            If (((myProps.bStatus And 6) = 6) Or myProps.bValue = True) And (myProps.bShape <> lv_CustomFlat And myProps.bShape <> lv_RoundFlat) Then
                SelectClipRgn ButtonDC.hdc, ButtonDC.ClipBorder
                myProps.bGradient = lv_Top2Bottom
...
Private Sub DrawButton_Win95(polyPts() As POINTAPI, polyColors() As Long, ActiveStatus As Integer, lastClipRgn As Integer)
    ...
    DrawButtonBackground backShade, ActiveStatus, lastClipRgn, ConvertColor(myProps.bGradientColor), adjHoverColor
...
Private Sub DrawButton_Macintosh(polyPts() As POINTAPI, polyColors() As Long, ActiveStatus As Integer, lastClipRgn As Integer)
    ...
    If ((myProps.bStatus And 6) = 6 And myProps.bMode = lv_CommandButton) Or myProps.bValue = True Then
...
Private Sub DrawButton_Hover(polyPts() As POINTAPI, polyColors() As Long, ActiveStatus As Integer, lastClipRgn As Integer)
    ...
    If ((myProps.bShape = lv_RoundFlat And (bTimerActive = True Or ((myProps.bStatus And 6) = 6))) Or myProps.bShape > lv_RoundFlat) Or (myProps.bShape = lv_CustomFlat And myProps.bValue = False) Then
        polyColors(12) = 0
    Else
        lastClipRgn = 1
